...
import { updateLinenType } from "@/actions/update-linen-type";
...
interface EditLinenTypeFormProps {
  linenTypeId: string;
  defaultValues: {
    name: string;
...
export function EditLinenTypeForm({
  linenTypeId,
  defaultValues,
  onSuccess,
}: EditLinenTypeFormProps) {
  const router = useRouter();
  const form = useForm<z.infer<typeof updateLinenTypeSchema>>({
    resolver: zodResolver(updateLinenTypeSchema),
    defaultValues: {
  ...
  async function onSubmit(values: z.infer<typeof updateLinenTypeSchema>) {
    const result = await updateLinenType({
      id: linenTypeId,
      ...values,
    });
    ...
    toast.success("Tipo de enxoval atualizado com sucesso");
    router.refresh();
    onSuccess?.();
  }
